SELECT *
FROM studente
WHERE corsocitta= 'MO' AND a_corso=2;
...
-- Selezionare i prodotti che nell’anno 2015 sono stati forniti da almeno un fornitore di Modena
SELECT DISTINCT prodotto.codp, prodotto.descrizione, prodotto.prezzo FROM prodotto
WHERE codp IN (SELECT codpFROM fornisce WHERE anno = 2015 AND codf IN (SELECT codf FROM fornitore WHERE citta = 'Modena'));

-- Selezionare i prodotti che nell’anno 2015 sono stati forniti da almeno un fornitore di Modena
SELECT codf, anno, SUM(qty) AS 	QTY_TOTALE FROM fornisce GROUP BY anno, codf;
